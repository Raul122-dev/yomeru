...
def _discover_custom_fonts() -> list[Path]:
    """Discover all custom font files in assets/fonts/ for fallback use."""
...
def _get_font(style: str, size: int) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    ...
    # Priority: style-specific list, then all custom fonts, then system fallbacks
    candidates = list(_FONT_STYLES.get(style, _FONT_STYLES["regular"]))
    # Append any custom fonts not already in the list
    custom = _discover_custom_fonts()
    for p in custom:
