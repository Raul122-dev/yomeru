...
import { AlgorithmCompareModal } from "./AlgorithmCompareModal";
import type { StageLog, MatchEvent, OrphanedRegion } from "../../lib/types";
...
interface RegionDisplay {
  ...
  label: string;
  score: number;
  status: "matched" | "fallback" | "orphaned";
  dialogue_index?: number;
  dialogue_text?: string;
  ocr_text?: string | null;
}
...
const STATUS_COLOR = {
  matched: "#22c55e", // green — direct VLM match
  fallback: "#fbbf24", // yellow — fallback/rescue match
  orphaned: "#ef4444", // red — no match
} as const;
...
export function MatchingEditor({
  ...
  const regions: RegionDisplay[] = (s2.regions ?? []).map((r) => {
    ...
    return {
      ...
      label: r.label,
      score: r.score,
      status: match ? (match.match_type === "fallback" ? "fallback" : "matched") : "orphaned",
  ...
  const isRunning = rerunProgress.status === "running";
        ...
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[hsl(var(--border))] bg-[hsl(var(--bg-subtle))]">
          ...
          <div className="flex items-center gap-2 text-[10px]">
            <span className="flex items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-green-500" /> direct
            </span>
            <span className="flex items-center gap-1">
              <span className="h-2 w-2 rounded-full bg-yellow-400" /> fallback
            </span>
            <span className="flex items-center gap-1">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 rounded border border-[hsl(var(--border))] px-2 py-1 text-[11px] text-[hsl(var(--accent2))] hover:bg-[hsl(var(--accent2)/.08)] transition-colors"
              title="Compare with algorithm-only matching"
            >
              <FlaskConical size={10} /> algorithm
            </button>
            <button
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="p-3 space-y-2 border-b border-[hsl(var(--border))]">
              {[
                ...
                {
                  label: "fallback",
                  count: regions.filter((r) => r.status === "fallback").length,
