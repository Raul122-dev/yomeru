...
from core.models import DEFAULT_FORMAT, PageAnalysis, format_context_str
...
def _build_language_block(
    ...
    if translate and target_language:
        lines.append(f"\n**Translation enabled: {source_language} → {target_language}**")
        ...
        lines.append("\nDo NOT include `text_translated`. Text extraction only.")
...
def _build_user_text(
    ...
    lang_block = _build_language_block(source_language, translate, target_language, ui_language)
...
def analyze_page(
    ...
    if saved_detections_dir is not None:
        ...
        try:
            ...
            from core.annotator import annotate_from_detections, load_page_detections_list
            ...
            if regions:
                ...
                region_lines = "\n".join(
                    f"  [{r['id']}] {r['label']} at ({r['x1']},{r['y1']})-({r['x2']},{r['y2']})"
                    for r in regions
                )
                detection_context = (
                    f"\n\n## Detected text regions ({n} total)\n"
                    f"The following regions have been detected and numbered on the image:\n"
                    f"{region_lines}\n"
                    f"For each dialogue, set `region_id` to the number of the region that contains it."
                )
    ...
    print(f"  → litellm  model={model!r}  api_base={api_base!r}  lang={source_language}"
          + (f" → {target_language}" if translate and target_language else "")
