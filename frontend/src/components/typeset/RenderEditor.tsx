...
import type { RenderEvent, StageLog } from "../../lib/api";
...
async function reRenderPage(
  ...
  const res = await fetch(
    `/api/runs/${runId}/typeset/stages/rendering/${pageNum}`,
    {
      method: "POST",
...
export function RenderEditor({
  ...
  const buildOverrideList = (): RenderOverride[] =>
    Object.entries(overrides)
      .filter(([, v]) => Object.keys(v).length > 0)
      .map(([k, v]) => ({ dialogue_index: parseInt(k), ...v }));

  const reRenderMutation = useMutation({
    mutationFn: () => reRenderPage(runId, pageNum, buildOverrideList()),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["stage-log", runId, pageNum] });
  ...
  const saveMutation = useMutation({
    mutationFn: () => saveRenderOverrides(runId, pageNum, buildOverrideList()),
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[hsl(var(--border))] bg-[hsl(var(--bg-subtle))]">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            <RefreshCw
              size={10}
              className={reRenderMutation.isPending ? "animate-spin" : ""}
            />
            {reRenderMutation.isPending ? "re-rendering…" : "▶ re-render page"}
          </button>
        </div>
      </div>
